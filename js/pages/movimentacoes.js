...
class MovimentacoesPage {
    ...
    filterMovimentacoes(searchTerm) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (cells.length > 2) {
                const codigoText = (cells[1]?.textContent || '').toLowerCase();
                const descricaoText = (cells[2]?.textContent || '').toLowerCase();

                if (codigoText.includes(term) || descricaoText.includes(term)) {
                    found = true;
                }
            }
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="variante">
                                <option value="">Selecione...</option>
                                ${movimentacao && movimentacao.variante ? `<option value="${movimentacao.variante}" selected>${movimentacao.variante}</option>` : ''}
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6">
    ...
    async saveMovimentacao() {
        try {
            ...
            const dataVencimentoValue = dataVencimentoField ? dataVencimentoField.value.trim() : '';
            const dataVencimento = dataVencimentoValue ? Utils.parseDateOnly(dataVencimentoValue) : null;
    ...
    applyFilters() {
        ...
        if (filters.data) {
            const filterDate = Utils.parseDateOnly(filters.data);
            if (filterDate) {
                filteredMovimentacoes = filteredMovimentacoes.filter(mov => {
                    const movDate = Utils.parseDateOnly(mov.data);
                    return movDate && movDate.toDateString() === filterDate.toDateString();
                });
            }
        }
        ...
        if (filters.vencimento) {
            const filterDate = Utils.parseDateOnly(filters.vencimento);
            if (filterDate) {
                filteredMovimentacoes = filteredMovimentacoes.filter(mov => {
                    if (!mov.data_vencimento) return false;
                    const movDate = Utils.parseDateOnly(mov.data_vencimento);
                    return movDate && movDate.toDateString() === filterDate.toDateString();
                });
            }
        }
