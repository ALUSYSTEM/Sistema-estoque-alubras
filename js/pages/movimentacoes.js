...
class MovimentacoesPage {
    ...
    async loadMovimentacoes() {
        try {
            ...
            this.movimentacoes.forEach((mov, index) => {
                ...
                console.log('  - Projeto descrição:', mov.projeto?.descricao);
                console.log('  - Projeto existe?', !!mov.projeto);
            });
            console.log('===============================');
                        ...
                        <table class="table table-bordered table-hover mb-0" id="movimentacoesTable">
                            <thead class="table-dark sticky-top">
                                ...
                                <tr class="table-light">
                                    ...
                                    <th>
                                        <input type="text" class="form-control form-control-sm" id="filter-codigo" placeholder="Filtrar código">
                                    </th>
                                    <th>
    ...
    renderMovimentacoesTable() {
        ...
        return this.movimentacoes.map(mov => {
            ...
            <tr>
                ...
                <td>
                    <small>${mov.descricao_planilha || mov.produto?.descricao || 'N/A'}</small>
                </td>
                <td>${mov.variante || '-'}</td>
                <td>${mov.tamanho || '-'}</td>
                <td>${mov.lote || '-'}</td>
    ...
    viewMovimentacao(movimentacaoId) {
        ...
        const content = `
            <div class="row">
                ...
                <div class="col-md-6">
                    <p><strong>Produto:</strong> ${movimentacao.produto?.codigo}</p>
                    <p><strong>Descrição:</strong> ${movimentacao.descricao_planilha || movimentacao.produto?.descricao || 'N/A'}</p>
    ...
    async parseMovimentacaoRow(row, columnMap, produtoMap, localizacaoMap, projetoMap) {
        try {
            ...
            if (columnMap.data !== undefined && row[columnMap.data]) {
                ...
                if (dataValue) {
                    ...
                    if (typeof dataValue === 'number') {
                        // Excel serial date - corrigido para fuso horário local
                        // Excel data começa em 01/01/1900, JavaScript em 01/01/1970
                        // Diferença de dias: 25569
                        // Mas precisamos ajustar para fuso horário local
                        const excelEpoch = new Date(1899, 11, 30); // 30 de dezembro de 1899
                        data = new Date(excelEpoch.getTime() + dataValue * 86400 * 1000);
                        console.log(`Data Excel ${dataValue} convertida para:`, data.toLocaleDateString());
                ...
                if (dataVencimentoRaw && dataVencimentoRaw.toString().trim() !== '') {
                    try {
                        ...
                        if (typeof dataVencimentoRaw === 'number') {
                            // Excel serial date - corrigido para fuso horário local
                            const excelEpoch = new Date(1899, 11, 30); // 30 de dezembro de 1899
                            dataVencimento = new Date(excelEpoch.getTime() + dataVencimentoRaw * 86400 * 1000);
                            console.log(`Data de vencimento Excel ${dataVencimentoRaw} convertida para:`, dataVencimento.toLocaleDateString());
                        } else if (dataVencimentoRaw instanceof Date) {
                            dataVencimento = dataVencimentoRaw;
                        } else {
            ...
            // Buscar descrição original da planilha
            const descricaoPlanilha = row[columnMap.descricao] ? row[columnMap.descricao].toString().trim() : null;
            ...
            const movimentacaoData = {
                ...
                usuario_nome: window.currentUserData ? window.currentUserData.nome : 'Sistema',
                descricao_planilha: descricaoPlanilha // Descrição original da planilha
            };
    ...
    async clearAllMovimentacoes() {
        try {
            Utils.showMessage('Limpando todas as movimentações e estoque...', 'info');
            
            const result = await databaseManager.clearAllMovimentacoes();
            
            Utils.showMessage(`${result.count} movimentações e ${result.estoqueCount} registros de estoque foram excluídos com sucesso!`, 'success');
            ...
            Utils.showMessage('Erro ao limpar movimentações e estoque', 'error');
        }
    }
