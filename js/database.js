...
class DatabaseManager {
    constructor() {
        this.db = db;
        this.cache = new Map(); // Cache simples para melhorar performance
    ...
    // OTIMIZAÇÃO: Sistema de cache simples
    getFromCache(key) {
    ...
    // OTIMIZAÇÃO: Invalidar cache quando dados são modificados
    invalidateCache(pattern = null) {
        if (pattern) {
            // Invalidar cache específico por padrão
            ...
            this.cache.clear();
            console.log('Todo cache foi invalidado');
        }
    }
    ...
    // OTIMIZAÇÃO: Buscar apenas produtos específicos por IDs
    async getProdutosByIds(ids) {
        try {
            if (!ids || ids.length === 0) return [];
            
            // Firestore tem limite de 10 documentos por consulta 'in', então fazemos em lotes
            const batchSize = 10;
            const batches = [];
            ...
            for (let i = 0; i < ids.length; i += batchSize) {
                ...
                batches.push(
                    this.db.collection('produtos').where(firebase.firestore.FieldPath.documentId(), 'in', batch).get()
                );
            }
    ...
    async getEstoque(filters = {}) {
        try {
            // OTIMIZAÇÃO: Verificar cache primeiro para consultas sem filtros específicos
            const cacheKey = `estoque_${JSON.stringify(filters)}`;
            const cachedData = this.getFromCache(cacheKey);
            if (cachedData && Object.keys(filters).length === 0) {
                console.log('Retornando estoque do cache');
                return cachedData;
            }
            ...
            const produtoIds = Array.from(new Set(estoques.map(e => e.produto_id).filter(id => id)));
            const localizacaoIds = Array.from(new Set(estoques.map(e => e.localizacao_id).filter(id => id)));
            const projetoIds = Array.from(new Set(estoques.map(e => e.projeto_id).filter(id => id)));
            
            console.log('=== DEBUG ESTOQUE DATABASE ===');
            console.log('IDs de produtos:', produtoIds);
            console.log('IDs de localizações:', localizacaoIds);
            console.log('IDs de projetos:', projetoIds);
            
            // Buscar dados relacionados
            const [produtos, localizacoes, projetos] = await Promise.all([
                produtoIds.length > 0 ? this.getProdutosByIds(produtoIds) : Promise.resolve([]),
                localizacaoIds.length > 0 ? this.getLocalizacoesByIds(localizacaoIds) : Promise.resolve([]),
                projetoIds.length > 0 ? this.getProjetosByIds(projetoIds) : Promise.resolve([])
            ]);
            
            console.log('Produtos carregados:', produtos.length);
            console.log('Localizações carregadas:', localizacoes.length);
            console.log('Projetos carregados:', projetos.length);
            
            // Criar mapas para busca rápida
            const produtoMap = new Map(produtos.map(p => [p.id, p]));
            const localizacaoMap = new Map(localizacoes.map(l => [l.id, l]));
            const projetoMap = new Map(projetos.map(p => [p.id, p]));
            
            // Associar dados relacionados aos registros de estoque
            estoques = estoques.map(estoque => ({
                ...estoque,
                produto: produtoMap.get(estoque.produto_id),
                localizacao: localizacaoMap.get(estoque.localizacao_id),
                projeto: projetoMap.get(estoque.projeto_id)
            }));
            
            console.log('Estoques após merge:', estoques.slice(0, 3));
            console.log('=====================================');
            
            // Cache apenas para consultas gerais de estoque
            if (Object.keys(filters).length === 0) {
                this.setCache(cacheKey, estoques);
            }
    ...
    async updateEstoque(produtoId, localizacaoId, projetoId, lote, variante, tamanho, quantidade, dataVencimento = null) {
        try {
            ...
            // OTIMIZAÇÃO: Invalidar cache de estoque após atualização
            this.invalidateCache('estoque');
    ...
    async getMovimentacoes(filters = {}) {
        try {
            ...
            // OTIMIZAÇÃO: Buscar apenas os dados relacionados que realmente existem nas movimentações
            console.log('=== DEBUG CARREGAMENTO DADOS RELACIONADOS ===');
            console.log('IDs de produtos para buscar:', Array.from(produtoIds));
            console.log('IDs de localizações para buscar:', Array.from(localizacaoIds));
            console.log('IDs de projetos para buscar:', Array.from(projetoIds));
            
            const [produtos, localizacoes, projetos] = await Promise.all([
                produtoIds.size > 0 ? this.getProdutosByIds(Array.from(produtoIds)) : Promise.resolve([]),
                localizacaoIds.size > 0 ? this.getLocalizacoesByIds(Array.from(localizacaoIds)) : Promise.resolve([]),
                projetoIds.size > 0 ? this.getProjetosByIds(Array.from(projetoIds)) : Promise.resolve([])
            ]);
    ...
    async addMovimentacao(movimentacao) {
        try {
            ...
            this.invalidateCache('movimentacoes');
    ...
    async clearAllMovimentacoes() {
        try {
            console.log('Iniciando limpeza de todas as movimentações e estoque...');
            
            // Buscar todas as movimentações
            const movimentacoesSnapshot = await this.db.collection('movimentacoes').get();
            
            // Buscar todos os registros de estoque
            const estoqueSnapshot = await this.db.collection('estoque').get();
            
            if (movimentacoesSnapshot.empty && estoqueSnapshot.empty) {
                console.log('Nenhuma movimentação ou estoque encontrado para deletar');
                return { count: 0, estoqueCount: 0 };
            }
            ...
            // Adicionar movimentações ao batch
            movimentacoesSnapshot.docs.forEach((doc) => {
                batch.delete(doc.ref);
                movCount++;
                
                if ((movCount + estoqueCount) % batchSize === 0) {
                    batches.push(batch);
                    batch = this.db.batch();
                }
            });

            // Adicionar registros de estoque ao batch
            ...
            console.log(`${movCount} movimentações e ${estoqueCount} registros de estoque deletados com sucesso`);
            
            // Invalidar cache
            this.invalidateCache('movimentacoes');
            this.invalidateCache('estoque');
            
            return { count: movCount, estoqueCount: estoqueCount };
            
        } catch (error) {
            console.error('Erro ao limpar movimentações e estoque:', error);
            throw error;
        }
    }
    ...
    // OTIMIZAÇÃO: Buscar apenas localizações específicas por IDs
    async getLocalizacoesByIds(ids) {
        try {
            if (!ids || ids.length === 0) return [];
            ...
            for (let i = 0; i < ids.length; i += batchSize) {
                const batch = ids.slice(i, i + batchSize);
                batches.push(
                    this.db.collection('localizacoes').where(firebase.firestore.FieldPath.documentId(), 'in', batch).get()
                );
            }
            ...
            console.error('Erro ao buscar localizações por IDs:', error);
            throw error;
        }
    }
    ...
    // OTIMIZAÇÃO: Buscar apenas projetos específicos por IDs
    async getProjetosByIds(ids) {
        try {
            if (!ids || ids.length === 0) return [];
            ...
            for (let i = 0; i < ids.length; i += batchSize) {
                const batch = ids.slice(i, i + batchSize);
                batches.push(
                    this.db.collection('projetos').where(firebase.firestore.FieldPath.documentId(), 'in', batch).get()
                );
            }
            ...
            console.error('Erro ao buscar projetos por IDs:', error);
            throw error;
        }
    }
